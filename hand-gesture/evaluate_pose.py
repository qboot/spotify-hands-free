...
def process_img(known_finger_poses, img_name):
	...
	#if args.solve_by == 0:
	score_label = predict_by_geometry(keypoint_coord3d_v, known_finger_poses, 0.55)
	...
	file_save_path = os.path.join('../images_out', "{}.png".format(int(time.time())))
...
if __name__ == '__main__':
	...
	# network input
	image_tf = tf.compat.v1.placeholder(tf.float32, shape = (1, 240, 320, 3))
	...
	while True:
		...
		while True:
			...
			if not data:
				break
			# print(data)
			response = process_img(known_finger_poses, data)
